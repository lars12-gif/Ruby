...
# ==========================================
# 4. وظائف قواعد البيانات (Supabase بدلاً من SQLite)
# ==========================================
def hash_password(pwd):
    return hashlib.sha256(pwd.encode('utf-8')).hexdigest()

# تمت إزالة init_db لأن الجداول موجودة الآن في Supabase مباشرة
...
def fetch_user(username):
    try:
        ...
        if res.data:
            user = res.data[0]
            # نعيد الداتا بنفس الهيكلية القديمة لضمان عمل الكود
            return {
...
if not st.session_state["logged_in"]:
    ...
    # زر الانتقال للأوبلز في شاشة الدخول
...
# زر الانتقال للأوبلز في الواجهة الرئيسية
st.markdown(f'<a href="{OPALS_SITE_URL}" target="_blank" class="ruby-site-btn">✨ الانتقال إلى موقع نقاط التفاعل (Opal\'s) 🚀</a>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    res_all = supabase.table("users").select("balance").execute()
    all_balances = [x["balance"] for x in (res_all.data or [])]
    ...
    with col_s2:
        st.metric(label="👥 عدد حسابات الأعضاء", value=f"{total_users}")
...
with tabs[4]:
    ...
    with st.form("pwd_form"):
        ...
        if st.form_submit_button("✏️ تحديث كلمة السر"):
            ...
            real_hash = res_pwd.data[0]["password_hash"]
